...
const marketingInsideCheck = async (node) => {
    ...
    const countNode = await MatrixFive.findAll({
    ...
    const countRightMatrix = await MatrixFive.count({
      where: { parent_id: rightNode[0].id },
    });
  
    const countLeftMatrix = await MatrixFive.count({
...
module.exports = async (parentId) => {
    ...
    const parentOneStep = await MatrixFive.findOne({ where: { id: parentId } });
    // if (!parentOneStep.parent_id) {
    //   return false;
    // }
    const parentTwoStep = await MatrixFive.findOne({
